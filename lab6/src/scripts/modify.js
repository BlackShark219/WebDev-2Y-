...
form.addEventListener('submit', (event) => {
  ...
  if (CheckInput()) {
    butt.disabled = true
    MakeChange().then(()=>document.location.href='index.html')}
  })
...
async function MakeChange(){
  ...
  let review = {
    id: response[localStorage.number].id,
    photoCard: localStorage.picture,
    photoCard_title: title.value,
    rating: rating.value
  }
  return BookR.changeInfo(review,localStorage.number)
}


function makeForm() {
  return'<div class="mui-col-lg-5 mui-col-md-5 photobar" style="text-align: center"><img src="' + localStorage.picture+ '" class="photobar-item"/></div>'+
        '<form id="form">\n' +
